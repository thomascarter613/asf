---
title: "ADR-0007: Adopt Polyglot Persistence with Qdrant as the Specialized Vector Retrieval Index"
document_id: "asf-adr-0007-polyglot-persistence-and-qdrant-retrieval"
adr_number: "0007"
version: "1.0.0"
status: "accepted"
...
# ADR-0007: Adopt Polyglot Persistence with Qdrant as the Specialized Vector Retrieval Index
...
The project requires relational state, document-shaped state, vector retrieval, and raw artifact storage.
...
PostgreSQL would store relational state, document-shaped state through JSONB, and vectors through pgvector.
...
MongoDB would store document state, relational-like state, and vector search indexes.
...
PostgreSQL and MongoDB would be canonical stores, with retrieval implemented through PostgreSQL, MongoDB, or application-layer logic.
...
makes future dense, sparse, and hybrid retrieval evolution less clean
```
...
Pinecone would be the managed vector database.
...
Pinecone remains a possible future managed option if operational requirements outweigh local-first and self-hosting requirements.

### Option 5: Weaviate

Weaviate would be the vector and semantic search platform.
...
Milvus would be the vector database.
...
more infrastructure than needed for early Foundry and Charon milestones
...
Milvus remains a future option if the system reaches vector-search scale that justifies heavier infrastructure.

### Option 7: OpenSearch or Elasticsearch

OpenSearch or Elasticsearch would serve as the primary search and vector layer.
...
LanceDB would be the vector store.
...
better fit for local, lakehouse, and dataset-oriented workflows than the central Foundry retrieval subsystem
```
...
Chroma would be the vector store.
...
PostgreSQL and MongoDB serve as canonical databases, and Qdrant serves as the specialized vector retrieval engine.
...
Qdrant provides the best balance of specialization, operational realism, architectural clarity, local development, and long-term production viability.
...
```text
1. Add local Docker Compose services for PostgreSQL, MongoDB, Qdrant, and MinIO.
2. Add typed environment validation.
3. Define canonical IDs and cross-store reference rules.
4. Define MongoDB document schemas for parsed documents, chunks, context packets, and indexing manifests.
5. Define PostgreSQL tables for projects, repositories, indexing runs, jobs, and audit events.
6. Define Qdrant collection and payload schemas.
7. Add an indexing worker.
8. Add a retrieval service.
9. Add an end-to-end retrieval smoke test.
10. Add a Qdrant rebuild command.
11. Add documentation for backup, restore, and rebuild operations.
```
...
PostgreSQL plus pgvector becomes sufficient and dramatically simpler.
...
Only canonical stores own truth.
```